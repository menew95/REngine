...
// 표준 방법입니다. 이 DLL에 들어 있는 파일은 모두 명령줄에 정의된 SERIALIZE_EXPORTS 기호로
// 컴파일됩니다. 이 DLL을 사용하는 프로젝트에서는 이 기호를 정의할 수 없습니다.
// 이렇게 하면 소스 파일에 이 파일이 포함된 다른 모든 프로젝트에서는
// SERIALIZE_API 함수를 DLL에서 가져오는 것으로 표시되는 반면, 이 DLL에서는
...
class SERIALIZE_API CSerialize {
public:
	CSerialize(void);
	// TODO: 여기에 메서드를 추가합니다.
};

extern SERIALIZE_API int nSerialize;

SERIALIZE_API int fnSerialize(void);
